    ...
    <section id="experience" ref={ref} className="section-shell">
      <div className="section-container container mx-auto px-4 md:px-6">
        ...
        >
          ...
          <h2 className="section-title">Experience In Motion</h2>
          <p className="section-copy">
            A timeline of enterprise AI, data engineering, and production ML systems delivered across business-critical
            environments.
          </p>
        </motion.div>

        <motion.div variants={stagger(0.12)} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="timeline-wrap space-y-6 pl-0 md:pl-10">
          {experiences.map((experience, index) => {
            const tone = tones[index % tones.length];
            const darkTone = tone === 'slate' || tone === 'forest';

            return (
              <motion.div key={experience.company} variants={cardReveal} className="relative">
                <span className="timeline-node" aria-hidden="true" />
                <PortfolioCard tone={tone} className="p-6 md:p-7" interactive>
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                      <p className={`font-mono text-xs uppercase tracking-[0.11em] ${darkTone ? 'text-[#F5E6CC]/72' : 'text-[#2a2e23]/70'}`}>
                        {experience.company}
                      </p>
                      <h3 className="mt-1 text-2xl leading-tight">{experience.title}</h3>
                    </div>

                    <div className={`rounded-xl border px-3 py-2 text-xs font-mono ${darkTone ? 'border-[#F5E6CC]/28 bg-[#F5E6CC]/12' : 'border-[#2f3328]/20 bg-black/10'}`}>
                  ...
                  <p className={`mt-4 text-sm leading-relaxed ${darkTone ? 'text-[#F5E6CC]/85' : 'text-[#2e3328]/84'}`}>
                  ...
                  <div className="mt-5 grid gap-2 md:grid-cols-2">
                    {experience.details.map((detail, detailIndex) => (
                      <motion.div
                        ...
                        whileHover={{ x: 2 }}
                      >
                        <span className={`mt-1 h-2 w-2 rounded-full ${darkTone ? 'bg-[#F5E6CC]' : 'bg-[#24301e]'}`} />
                        <p className={`text-[0.86rem] leading-relaxed ${darkTone ? 'text-[#F5E6CC]/84' : 'text-[#2f342a]/82'}`}>{detail}</p>
                      </motion.div>
                    ))}
                  </div>
                </PortfolioCard>

      ...
      >
        <div className="e-card tone-forest nav-shell px-4 md:px-5 py-3">
          <div className="flex items-center justify-between gap-4">
            <a href="#home" className="inline-flex items-center gap-2 font-semibold tracking-wide text-[0.95rem]">
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#F5E6CC]/15 border border-[#F5E6CC]/25">
                <Leaf size={16} />
              </span>
              <span className="font-mono text-xs sm:text-sm uppercase">Bharath Portfolio</span>
            </a>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              exit="exit"
            >
              <motion.nav variants={stagger(0.06)} initial="hidden" animate="visible" className="e-card tone-forest nav-shell p-4">
                <ul className="space-y-2">
                  {navLinks.map((link, index) => (
                    <motion.li key={link.id} variants={{ hidden: { opacity: 0, x: -12 }, visible: { opacity: 1, x: 0, transition: { delay: index * 0.03 } } }}>
                      <a
                        href={link.href}
                        onClick={() => setIsOpen(false)}
